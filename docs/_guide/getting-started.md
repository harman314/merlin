...
description: Install Merlin, link your phone, and load chat history.
nav_order: 2
---
...
```sh
git clone https://github.com/harman314/merlin merlin
cd merlin
cargo install --path .
merlin
```
...
A desktop entry ships in `packaging/applications/merlin.desktop`.

## Link with your phone

Merlin links as a companion device, like WhatsApp Web. Start it and either:
...
network or be online to read messages already stored in Merlin.

## Message history

After linking, the phone sends recent history. The chat list appears within
seconds, and messages can take a few minutes to finish loading. Merlin stores
new messages in its own archive. When you scroll past the stored history,
Merlin asks your phone for older messages. The phone must be online.
