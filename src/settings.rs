...
/// Optional build-time GIPHY key from `MERLIN_GIPHY_KEY`.
/// The previous name remains accepted for existing build setups.
pub const BUILT_IN_GIPHY_KEY: Option<&str> = match option_env!("MERLIN_GIPHY_KEY") {
...
mod tests {
    ...
    fn round_trips_through_disk() {
        let dir = std::env::temp_dir().join(format!("merlin-settings-{}", std::process::id()));
