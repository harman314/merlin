...
mod tests {
    ...
    fn a_moving_subject_leaves_no_trace_behind() {
        ...
        let dir = std::env::temp_dir().join(format!("merlin-ghost-{}", std::process::id()));
    ...
    fn animated_webp_decodes_into_frames() {
        ...
        let dir = std::env::temp_dir().join(format!("merlin-anim-{}", std::process::id()));
        std::fs::create_dir_all(&dir).expect("dir");
    ...
    fn an_mp4_made_by_ffmpeg_decodes_in_process() {
        ...
        let dir = std::env::temp_dir().join(format!("merlin-mp4-{}", std::process::id()));
        std::fs::create_dir_all(&dir).expect("dir");
    ...
    fn a_still_webp_is_not_an_animation() {
        let dir = std::env::temp_dir().join(format!("merlin-still-{}", std::process::id()));
        std::fs::create_dir_all(&dir).expect("dir");
...
mod probe {
    ...
    /// Decodes the file in `MERLIN_MP4_PROBE`:
    /// `MERLIN_MP4_PROBE=some.mp4 cargo test --all-features probe -- --ignored --nocapture`.
    #[test]
    #[ignore = "needs a file to look at"]
    fn decodes_the_file_named_by_the_environment() {
        let Some(path) = std::env::var_os("MERLIN_MP4_PROBE") else {
            return;
        };
