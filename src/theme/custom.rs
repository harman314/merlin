...
pub(super) fn parse_palette(text: &str) -> Result<Palette, String> {
    ...
    // colours when importing one, while keeping explicit Merlin overrides.
    ...
    fn spawn(
        ...
        match std::thread::Builder::new()
            ...
            Err(error) => {
                ...
                self.problem = Some(
                    "Custom themes could not be loaded. Run merlin reload-themes to try again."
                        .into(),
                );
            }
        }
...
mod custom_theme_tests {
    ...
    fn reloads_coalesce_and_never_publish_a_superseded_palette() {
        let dir =
            std::env::temp_dir().join(format!("merlin-theme-reload-{}", rand::random::<u64>()));
    ...
    fn discovery_sorts_valid_files_and_skips_invalid_files() {
        let dir = std::env::temp_dir().join(format!("merlin-themes-{}", rand::random::<u64>()));
    ...
    fn reads_are_bounded_and_cannot_escape_to_other_files() {
        let root =
            std::env::temp_dir().join(format!("merlin-theme-bounds-{}", rand::random::<u64>()));
    ...
    fn catalog_limits_keep_the_saved_selection_without_an_arbitrary_partial_listing() {
        let dir =
            std::env::temp_dir().join(format!("merlin-theme-catalog-{}", rand::random::<u64>()));
        std::fs::create_dir(&dir).unwrap();
