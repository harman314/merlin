...
impl Setup {
    pub(super) fn discover() -> Option<Self> {
        ...
        Some(Self {
            assets: executable.parent()?.parent()?.join("share/merlin/omarchy"),
    ...
    fn current_theme_with(
        ...
        let rendered = current.join("merlin.json");
        let text = if rendered.is_file() {
            read_small(&rendered)?
        } else {
            let custom = self.home.join(".config/omarchy/themed/merlin.json.tpl");
            let template = if custom.is_file() {
                read_small(&custom)?
            } else {
                include_str!("../../contrib/omarchy/merlin.json.tpl").to_owned()
            };
    ...
    pub(super) fn install(&self, themes: &Path) -> io::Result<()> {
        ...
        let template = read_small(&self.assets.join("merlin.json.tpl"))?;
        let hook = read_small(&self.assets.join("merlin-theme"))?;
        let template_path = config.join("themed/merlin.json.tpl");
        create_only(&template_path, template.as_bytes(), 0o644)?;
        create_only(
            &config.join("hooks/theme-set.d/merlin-theme"),
            hook.as_bytes(),
            0o755,
        )?;
...
fn create_only(destination: &Path, bytes: &[u8], mode: u32) -> io::Result<()> {
    ...
    let temporary = parent.join(format!(".merlin-setup-{:016x}", rand::random::<u64>()));
...
mod tests {
    ...
    const TEMPLATE: &str = include_str!("../../contrib/omarchy/merlin.json.tpl");
    const HOOK: &str = include_str!("../../contrib/omarchy/merlin-theme");
    ...
    impl Scratch {
        fn new() -> Self {
            let path = std::env::temp_dir()
                .join(format!("merlin omarchy setup {}", rand::random::<u64>()));
    ...
    fn setup_requires_both_a_package_and_an_omarchy_desktop() {
        ...
        let setup = Setup {
            assets: root.0.join("package/share/merlin/omarchy"),
    ...
    fn setup_is_per_user_and_preserves_customizations_and_preferences() {
        ...
        fs::write(setup.assets.join("merlin.json.tpl"), TEMPLATE).unwrap();
        fs::write(setup.assets.join("merlin-theme"), HOOK).unwrap();
        fs::write(current.join("merlin.json"), "{}").unwrap();
        let settings = themes.parent().unwrap().join("settings.json");
        fs::write(&settings, "existing preferences").unwrap();
        setup.install(&themes).unwrap();
        let template = config.join("themed/merlin.json.tpl");
        let hook = config.join("hooks/theme-set.d/merlin-theme");
