        ...
        match &*event {
            ...
            E::ClientOutdated(_) => {
                self.set_status(LinkStatus::Failed(
                    "WhatsApp rejected this version of Merlin. Update the app".to_owned(),
                ));
            }
            E::Messages(batch) => {
...
mod receipt_tests {
    ...
    pub(super) fn worker() -> (
        ...
        let (wa_sender, wa_events) = mpsc::unbounded_channel();
        let root = std::env::temp_dir().join(format!("merlin-worker-test-{}", std::process::id()));
        let worker = Worker {
