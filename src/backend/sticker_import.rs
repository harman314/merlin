...
mod tests {
    ...
    fn a_wastickers_archive_becomes_a_named_pack() {
        let root = std::env::temp_dir().join(format!("merlin-packs-{}", std::process::id()));
    ...
    /// Imports Signal's example pack from the live CDN. `MERLIN_TEST_PACK`
    /// can provide another signal.art link.
    #[test]
    #[ignore = "network"]
    fn fetches_a_real_pack_from_signal_on_this_machine() {
        let root = std::env::temp_dir().join(format!("merlin-signal-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&root);
        let url = std::env::var("MERLIN_TEST_PACK").unwrap_or_else(|_| {
