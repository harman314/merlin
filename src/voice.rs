...
mod tests {
    ...
    /// Decodes the file in `MERLIN_OGG_PROBE`:
    /// `MERLIN_OGG_PROBE=note.ogg cargo test voice::tests::probe -- --ignored --nocapture`.
    #[test]
    #[ignore = "needs a file to look at"]
    fn probe() {
        let Some(path) = std::env::var_os("MERLIN_OGG_PROBE") else {
            return;
        };
