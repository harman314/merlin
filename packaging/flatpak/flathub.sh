...
manifest = yaml.safe_load((here / "rocks.merlin.Merlin.yml").read_text())
manifest["modules"][0]["sources"][0] = {
    "type": "git", "url": "https://github.com/harman314/merlin.git", "commit": commit
}
(out / "rocks.merlin.Merlin.yml").write_text(yaml.safe_dump(manifest, sort_keys=False))
meta = ET.parse(here / "rocks.merlin.Merlin.metainfo.xml")
...
ET.SubElement(release, "url").text = f"https://github.com/harman314/merlin/releases/tag/v{version}"
ET.indent(meta, space="  ")
meta.write(out / "rocks.merlin.Merlin.metainfo.xml", encoding="UTF-8", xml_declaration=True)
PY
