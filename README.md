...
**WhatsApp, native and fast, and one day smart.** Merlin is a WhatsApp client
...
companion device, and has no browser engine. It opens in under a second and
idles around 150 MB, against 1.13 GB for WhatsApp Web and its Chromium
processes.

Merlin is a fork of [ZapFast](https://github.com/crmne/zapfast) by Carmine
Paolino, which is itself a sibling of [Spotifast](https://spotifast.rocks).
Merlin keeps ZapFast's speed and adds attachment preview in the app, pasting
several files at once, and an agent layer to come. Everything good about the
foundation is Carmine's work, under MIT.

Merlin installs beside ZapFast rather than over it. It keeps its own settings,
archive, and keyring entry, and links to your phone as a separate device, so
you can run both.

![Merlin showing a chat with a photo, a document, a voice message, a quoted reply, and a link](docs/screenshot.png)

See **[github.com/harman314/merlin](https://github.com/harman314/merlin)** for downloads and guides.
...
- **Conversations.** See replies, reactions, edits, deleted messages, read
  ...
  do not change that list. If the original recipients are unknown, Merlin
...
- **View attachments.** Merlin downloads files up to 64 MB automatically or
  ...
  If an attachment has expired, Merlin asks your
...
- **Polls.** Use the checklist button beside the paperclip to create a poll with
  ...
  the protocol library's poll API, so Merlin blocks it instead of ignoring the timer.
...
- **Runs in the background.** Closing the window keeps Merlin linked in the
...
- **Desktop notifications.** Get notifications with the chat picture when you
  ...
  identify Merlin as the sender and show chat pictures as small circular icons;
  ...
  the installed Merlin application's identity without an application chooser;
  unregistered development builds skip notifications if that identity is unavailable.
- **Update notices.** Merlin checks GitHub once a day and shows a download
...
Merlin has no published releases yet. Build it from source.

### From source

Merlin needs Rust, a C/C++ toolchain, CMake and Perl (for bundled OpenSSL).
...
conflicts that make no progress. Merlin does not reset your session to recover
a collection.

The packaging recipes under `packaging/` and `native-packages.yaml` still
describe repositories and taps that do not exist yet. Fill them in before
publishing anything.
...
If the keyring is locked or unavailable, unlock it and click Retry; Merlin keeps
...
Merlin needs Rust, a C/C++ toolchain, CMake and Perl (for bundled OpenSSL). `rust-toolchain.toml` pins the exact version. On Linux,
...
| What | Linux | Notes |
| --- | --- | --- |
| Settings | `~/.config/merlin/settings.json` | JSON, safe to edit |
| Device keys | `~/.local/state/merlin/session.db` | Owned by whatsapp-rust; deleting it unlinks |
| Messages | `~/.local/state/merlin/archive.db` | SQLCipher-encrypted SQLite, unlocked by the OS keyring; raw messages retain attachment keys |
| Attachments, avatars | `~/.cache/merlin/` | Safe to delete |
| Saved stickers and packs | `~/.local/state/merlin/stickers/` | Plain WebP files; each pack is a folder |
| Log of the last run | `~/.local/state/merlin/merlin.log` | `--verbose` for more |

macOS and Windows use the standard platform directories selected by the
`directories` crate. On first start, Merlin moves settings, the linked session,
message archive, saved stickers, caches, and window state from `fastsapp`
(or the earlier `fastwhatsapp`) paths. Existing Merlin directories take
precedence and are never overwritten. Quit FastsApp before starting Merlin;
...
On Linux and macOS, Merlin restricts its configuration, state, and cache
...
platforms, use `merlin reload-themes` after editing. The command also works while
...
Merlin checks GitHub once a day when **Check for updates** is enabled.
...
and the macOS app in Applications. Keep `merlin-portable.txt` beside a portable
...
```sh
MERLIN_GIPHY_KEY=your-key cargo build --release
```
...
./target/debug/merlin --demo-tour --demo-size 1280x800
```

The **Merlin Demo** window waits for **Space**. The 41-second tour starts with
...
in Merlin. Recording has no audio unless you explicitly enable desktop or
...
Merlin is an unofficial client and is not affiliated with WhatsApp or
